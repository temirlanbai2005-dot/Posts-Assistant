...
import { 
  ...
  Share2, AlertCircle, Sparkles, Microscope, Layers, Trash2, Languages, ChevronDown
} from 'lucide-react';
...
const PostGenerator: React.FC<PostGeneratorProps> = ({ settings, initialTopic, initialPlatform }) => {
  ...
  // Platform selected specifically for the "Idea Generator"
  const [ideaPlatform, setIdeaPlatform] = useState<SocialPlatform>(SocialPlatform.LinkedIn);
  ...
  useEffect(() => {
    ...
    if (initialPlatform) {
        ...
        setIdeaPlatform(initialPlatform);
    }
  ...
  const handleBrainstorm = async () => {
    ...
    try {
        const idea = await generateCreativeIdea(settings, ideaPlatform);
        setTopic(idea);
    } catch(e) {
        setTopic("💸 Warning: The cheapest 3D quote is usually the most expensive choice in the long run.");
    } finally {
        ...
        <div className="space-y-3 flex-1">
          <div className="flex justify-between items-center gap-2">
             ...
             <div className="flex items-center gap-1 bg-slate-800 rounded-lg p-0.5 border border-slate-700">
                <div className="relative">
                   <select 
                      value={ideaPlatform} 
                      onChange={(e) => setIdeaPlatform(e.target.value as SocialPlatform)}
                      className="appearance-none bg-transparent text-[10px] text-slate-300 pl-2 pr-4 py-1 focus:outline-none cursor-pointer hover:text-white"
                   >
                       {SUPPORTED_PLATFORMS.map(p => (
                           <option key={p.id} value={p.id} className="bg-slate-900">{p.id}</option>
                       ))}
                   </select>
                   <ChevronDown className="w-3 h-3 text-slate-500 absolute right-1 top-1.5 pointer-events-none" />
                </div>
                <div className="w-px h-3 bg-slate-600"></div>
                <button 
                  onClick={handleBrainstorm}
                  disabled={isBrainstorming}
                  className="text-[10px] px-2 py-1 text-indigo-400 hover:text-indigo-300 flex items-center gap-1 transition-colors font-medium"
                >
          ...
          <div className="relative h-full min-h-[160px]">
            <textarea
                ...
                placeholder={`Get a provocative ${ideaPlatform} idea or write your own...`}
            />
            {isBrainstorming && (
                <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm rounded-xl flex items-center justify-center text-sm text-indigo-300">
                    Cooking up a hot take for {ideaPlatform}...
                </div>
            )}
          </div>
        </div>
